...
- `product/create` 与 `product/update` 的 `desc` 按字符串发送，保留逗号、换行和空字符串，避免被错误转换为数组；使用 `autoFill` 更新时保留原描述。
- `feedback/close` 将 `confirmClose` 放入查询参数，不再写入 JSON 请求体，避免服务端将其当作反馈表字段。
- `bug/confirm` 自动预读详情并补齐省略的状态、指派人、类型、优先级、截止日期和抄送人，避免确认时重置这些字段；显式值保持优先级，字段齐全时不额外读取。预读失败或详情不完整时终止确认，此行为不依赖 `autoFill`。
- `product/create`、`product/update`、`feedback/close` 与 `bug/confirm` 将 HTTP 200 的 PHP/SQL 错误文本和空响应判为失败，保留原始响应文本，并支持通过 `throwOnFail` 抛出 `E_API_FAILED`。

### 变更

- `bug/confirm` 拒绝空状态及特殊指派人值 `closed`；确认指派人为 `closed` 的已关闭 Bug 时，需显式提供有效的 `assignedTo`，省略 `status` 仍保留已关闭状态。

### 测试

- 增加产品描述保留、反馈关闭参数路由、Bug 确认字段补齐与失败中止，以及写接口异常响应、原始响应和 HTTP 错误的回归覆盖。
- 真实环境测试增加确认 Bug 后仍保持激活状态的断言。

### 文档

- 明确三类需求的评审人字段在无需评审时可以省略，并重新生成产品与需求模块文档。
- 补充确认 Bug 的预读规则和已关闭 Bug 的处理示例，以及异常写响应的原文位置与回读后再决定是否重试的说明。

## 0.6.9 - 2026-09-12

### 新增

- 新增 `ModuleAction.beforeRequest` 及对应回调类型，支持在版本检查、`autoFill` 和参数解析完成后异步调整请求路径、查询参数、请求体等数据，普通请求和自定义请求均使用处理后的请求描述。

### 修复

- `story`、`requirement`、`epic` 的创建与变更根据有效评审人设置 `status: 'reviewing'`，未提供有效评审人时设置 `needNotReview: '1'`；补齐 `requirement/change` 的评审人字段，并允许省略评审人。

### 测试

- 增加 `beforeRequest` 的定义校验、异步执行与浅合并、上传准备、响应处理、错误传递及 `autoFill` 顺序的回归覆盖，并验证三类需求的评审参数处理。
- 扩展真实环境测试，覆盖更多对象的增删改查、状态流转、转化操作、不同范围的列表和四类文档空间，按依赖顺序清理临时数据，并输出动作、路径、失败与未覆盖项统计。
- 真实环境项目测试复用有效 Scrum 流程，支持通过 `ZENTAO_WORKFLOW_GROUP` 指定流程；加强 HTML 错误页、异常结果和清理失败的识别，避免测试误报成功。

### 文档

- 补充 `beforeRequest` 的调用时机、补丁语义与示例，以及真实环境测试的配置、覆盖报告和数据保留说明，重新生成 SDK 参考与需求模块文档。

## 0.6.8 - 2026-09-06

### 新增

- 公开 `ZentaoClient.fetch()`，支持使用完整 URL 复用客户端传输层，并显式传入 Token、缓存与凭据选项。
- 新增 `ModuleAction.request` 自定义请求回调及对应类型，在版本检查、参数解析和请求体准备后接管请求发送，继续复用结果提取、分页、上传校验与 `autoFill` 流程。
- `ZentaoClient.fromProfile()` 支持 `{ activate: false }`，只读恢复客户端，不切换当前账号或更新使用时间，适用于可读但不可写的存储。

### 修复

- 重新登录同一账号时保留已有自定义字段和未被显式覆盖的偏好，持久化的 `timeout` / `insecure` 与实际请求保持相同的选项优先级。
- Profile 修改在 Node.js / Bun 中使用文件锁，在支持 Web Locks 的浏览器中共用写锁，避免同机进程或同源页面并发写入时丢失更新。
- 损坏的 Profile JSON 或根结构抛出 `E_PROFILE_STORAGE_INVALID` 并保留原内容；权限、容量等存储错误统一为 `E_PROFILE_STORAGE_UNAVAILABLE`，只读恢复与账号切换保持一致的错误语义。
- `getZentaoConfig()` 在浏览器中显式省略 Cookie 等凭据，确保配置请求匿名发送。

### 测试

- 增加 Profile 并发写入、锁回收与超时、偏好保留、只读恢复及存储错误覆盖，以及匿名配置请求和自定义 Action 回调的响应处理、上传与 `autoFill` 回归测试。

### 文档

- 补充 Profile 偏好、并发保护、只读恢复和匿名配置查询说明，澄清删除回退顺序与浏览器入口在 SSR 中的运行时行为，并重新生成 SDK 参考文档。
