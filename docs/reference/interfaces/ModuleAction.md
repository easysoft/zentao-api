...
| <a id="property-pagergetter"></a> `pagerGetter?` | `string` \| [`ModuleActionPagerGetterMap`](ModuleActionPagerGetterMap.md) \| `ModuleActionGetterFn`\<[`Pager`](Pager.md), `RequestProcessOptions`\> | 从原始响应中提取分页信息的位置或函数： 字符串为字段路径（支持 `a.b` 嵌套）、对象为字段映射、函数则接收原始响应与调用参数。 |
...
| <a id="property-resultgetter"></a> `resultGetter?` | `string` \| `Readonly`\<`Record`\<`string`, `string`\>\> \| `ModuleActionGetterFn`\<`unknown`, `RequestProcessOptions`\> | 从原始响应中提取业务数据的位置或函数： 字符串为字段路径（支持 `a.b` 嵌套）、对象为字段映射、函数则接收原始响应与调用参数。 |
